...
const byte currentPin1=            3;
const byte currentPin2=            0;
const byte currentPin3=            1;
const byte voltageAcPin=           2;
...
void setup()
{
  ...
  if (analogRead(currentPin1) > 0) {CT1 = 1; CT_count++;} else CT1=0;              // check to see if CT is connected to CT1 input, if so enable that channel
  if (analogRead(currentPin2) > 0) {CT2 = 1; CT_count++;} else CT2=0;              // check to see if CT is connected to CT2 input, if so enable that channel
  if (analogRead(currentPin3) > 0) {CT3 = 1; CT_count++;} else CT3=0;              // check to see if CT is connected to CT3 input, if so enable that channel
  ...
  // Calculate if there is an AC-AC adapter on analog input 2
  double vrms = calc_rms(voltageAcPin,1780) * (Vcal * (3.3/1024) );
  ...
  if (DEBUG==1)
  {
    ...
    Serial.print("CT 4 Cal "); Serial.println(0);
    delay(1000);
  ...
  ct1.current(currentPin1, Ical1);             // CT ADC channel 1, calibration.  calibration (2000 turns / 22 Ohm burden resistor = 90.909)
  ct2.current(currentPin2, Ical2);             // CT ADC channel 2, calibration.
  ct3.current(currentPin3, Ical3);             // CT ADC channel 3, calibration.
  ...
  if (ACAC)
  {
    ...
    ct1.voltage(voltageAcPin, Vcal, phase_shift);          // ADC pin, Calibration, phase_shift
    ct2.voltage(voltageAcPin, Vcal, phase_shift);          // ADC pin, Calibration, phase_shift
    ct3.voltage(voltageAcPin, Vcal, phase_shift);          // ADC pin, Calibration, phase_shift
  }
...
void loop()
{
  ...
  if (CT1 || ACAC) {                                          // Awlays sample for CT's if powered by AC-AC
    ...
    if (analogRead(currentPin1) == 0) emontx.power1=0;                  // CT disconnected
  }
  ...
  if (CT2 || ACAC) {
    ...
    if (analogRead(currentPin2) == 0) emontx.power2=0;    // CT disconnected
  }
  ...
  if (CT3 || ACAC) {
    ...
    if (analogRead(currentPin3) == 0) emontx.power3=0;    // CT disconnected
  }
  ...
  if (!ACAC){                                                                         // read battery voltage if powered by DC
    int battery_voltage=1024 * 0.681322727;                // 6.6V battery = 3.3V input = 1024 ADC
  ...
  if (DEBUG==1) {
    ...
    Serial.print(",ct4:"); Serial.print(0);
